...
import static com.tripadvisor.WeekCellDescriptor.RangeState.FIRST;
import static com.tripadvisor.WeekCellDescriptor.RangeState.FIRST_AND_LAST;
import static com.tripadvisor.WeekCellDescriptor.RangeState.LAST;
import static com.tripadvisor.WeekCellDescriptor.RangeState.MIDDLE;
import static com.tripadvisor.WeekCellDescriptor.RangeState.NONE;
import static com.tripadvisor.WeekCellDescriptor.RangeState.OPEN;
...
public class CalendarView extends FrameLayout {
    ...
    private OnFirstWeekChangeListener mFirstWeekChangeListener;
    ...
    public CalendarView(Context context, AttributeSet attrs) {
        ...
        ArrayList<Date> eventsArray = new ArrayList<>();
        if (isInEditMode()) {
            Calendar nextYear = Calendar.getInstance(mLocale);
            nextYear.add(Calendar.YEAR, 1);
            init(new Date(), nextYear.getTime(), eventsArray)
    ...
    public FluentInitializer init(Date minDate, Date maxDate, Locale locale, ArrayList<Date> eventsArray) {
        ...
        while ((mMonthCounter.get(MONTH) <= maxMonth // Up to, including the month.
            ...
            List<WeekCellDescriptor> weekCells = getWeekCells(week, mMonthCounter, eventsArray);
        ...
        setUpListView();
        setUpAdapter();
        mListView.setSelection(mCurrentWeek);
    ...
    private void setMonthTitle(Date date) {
        mMonthName.setText(setFirstLatterCap(mFullMonthNameFormat.format(date)));
    }
     ...
     */
    public FluentInitializer init(Date minDate, Date maxDate, ArrayList<Date> eventsArray) {
        return init(minDate, maxDate, new Locale("nl"),  eventsArray);
    }
    ...
    public void setLocale(Locale locale) {
        ...
        if (mMonthName != null) {
            ...
            mMonthName.setText(setFirstLatterCap(newMonthName));
        }
    }
...
//            }
        ...
        for (int i = 1, count = mDayNamesHeader.getChildCount(); i < count; i++) {
            ...
            if (i < daysPerWeek + 1) {
                ...
                assert label != null;
                label.setText(getResources().getStringArray(R.array.week_days)[i - 1]);
    ...
    private void onScroll(AbsListView view) {
        ...
        if (mFirstWeekChangeListener != null) {
            mFirstWeekChangeListener.onFirstWeekChangeSelected(child.getWeekView(), mCurrentMonthDisplayed);
        }
    ...
    private String setFirstLatterCap(String month) {
        StringBuilder sb = new StringBuilder(month);
        sb.setCharAt(0, Character.toUpperCase(sb.charAt(0)));
        return sb.toString();
    }

    public List<WeekCellDescriptor> getWeekCells(WeekDescriptor week, Calendar startCal, ArrayList<Date> eventsArray) {
        //#TODO change to locale language
        Calendar cal = Calendar.getInstance(mLocale);
        if (weekContensCurrentDay(startCal, cal)) {
            mCurrentWeek = mCells.size();
        }
        ...
        for (int c = 0; c < 7; c++) {
            ...
            boolean isToday = isEventDate(cal, eventsArray);
            //#TODO change the logic for the hightlighted cal
            boolean isHighlighted = false;//containsDate(getHighlightedCals(), cal);
...
//                }
    ...
    private Boolean weekContensCurrentDay(Calendar startCal, Calendar current) {
        Calendar nextWeek = Calendar.getInstance();
        nextWeek.setTime(startCal.getTime());
        nextWeek.add(DAY_OF_MONTH, 7);
        if (startCal.compareTo(current) == 0 || nextWeek.compareTo(current) == 0)
            return true;
        if (startCal.compareTo(current) == -1 && nextWeek.compareTo(current) == 1)
            return true;
        return false;
    }

    private Boolean isEventDate(Calendar cal, ArrayList<Date> events) {
        for (int i = 0; i < events.size(); i++) {
            if (events.get(i).compareTo(cal.getTime()) == 0) {
                return true;
            }
        }
        return false;
    ...
    public void setOnFirstWeekChangeListener(OnFirstWeekChangeListener listener) {
        mFirstWeekChangeListener = listener;
    }
    ...
    public interface OnFirstWeekChangeListener {
        void onFirstWeekChangeSelected(WeekView week, int currentMount);
    }
